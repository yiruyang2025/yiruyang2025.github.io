...
description: 4D Reconstruction and Tracking, Connectomics, CVG, USZ, SCAI
...
| **Aspect**                                                                                                            | **Unresolved Pain Point (Traditional 3DV / SfM)**                                                                                                                                                                  | **Why Existing Methods Are Not Enough**                                                                                                                                           | **Potential Directions to Solve**                                                                                                                                                                    | **Existing Efforts (Yes / Partial / No)**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1. Dynamic Scenes (non-static, moving objects or non-rigid motion)**                                                | Traditional SfM assumes static scenes — moving or non-rigid objects are treated as outliers or noise.                                                                                                              | Even with feature-metric refinement or robust SfM, rigid/static scene assumption breaks down; motion causes inconsistent matches, reconstruction fails.                           | Introduce **dynamic NeRF / scene flow + SfM** / **implicit dynamic scene reconstruction** that models motion/deformation over time.                                                                  | **Partial — yes.** For example, **D-NeRF** extends NeRF to dynamic scenes (non-rigid motion). ([CVF开放获取][1])  Also **Neural SDF Flow** proposes SDF flow to reconstruct dynamic scenes from multi-view video. ([openreview.net][2])                                                                                                                                                                                                                                                                                                 |
| **2. Illumination / Appearance / Time Variance (lighting change, appearance change across time)**                     | Even learned CNN feature-based matching/refinement can degrade under large lighting changes, seasonal/weather changes, or long time gaps.                                                                          | Feature spaces are often static, trained for “typical” appearance variation; they may not generalize across extreme lighting, material changes, or long-term temporal variation.  | Develop **time-conditioned feature encoders**, **appearance flow fields**, or **illumination-invariant / temporal-aware representations** integrated into SfM / reconstruction.                      | **Partial.** In neural-rendering literature there are works dealing with appearance and dynamic lighting in dynamic NeRF frameworks. For example, dynamic-scene NeRF variants handle varying appearance over time. ([arXiv][3])  But for purely geometry-based SfM + featurebased refinement + classic reconstruction, it's largely unexplored.                                                                                                                                                                                     |
| **3. Extreme Viewpoint / View Baseline (very wide-baseline, large viewpoint change)**                                 | With very large viewpoint changes, feature correspondence can fail; even CNN features may not represent the same 3D point consistently.                                                                            | Traditional matching + descriptor + refinement often can’t handle extreme view changes, large occlusion, or very different angles.                                                | Combine **semantic / language / prior knowledge** (e.g. object-/part-level priors, shape priors, diffusion priors) to enable **semantic-aware SfM** that can match beyond purely local appearance.   | **Partial.** There are efforts in the neural rendering / NeRF community: e.g. **A Semantically Aware Multi-View 3D Reconstruction Method for Urban Applications** integrates semantic information into multi-view reconstruction to improve robustness in challenging urban scenes. ([MDPI][4])  Also general surveys on semantic-aware NeRFs show growing interest. ([arXiv][5])                                                                                                                                                   |
| **4. Sparse–Dense Gap: Difficulty Bridging Sparse SfM and Dense Reconstruction / Representation**                     | Traditional SfM gives sparse point clouds; dense reconstruction (MVS / mesh / volume) exists but merging sparse and dense results reliably remains hard.                                                           | Sparse and dense methods use different representations; optimization objectives differ (sparse keypoints vs dense photometric/depth), making unified, robust pipelines difficult. | Use **unified implicit fields / feature fields / SDF / Gaussian-based representations** that can encode both sparse and dense geometry, bridging the gap.                                            | **Partial / emerging.** New methods based on implicit representations (SDF / neural fields / Gaussian Splatting) begin to show promise at unifying sparse–dense paradigms. For example, **Semantic-Consistent 3D Reconstruction via Gaussian Splatting** tries to combine semantic + Gaussian-based 3D reconstruction. ([ISPRS Archives][6])                                                                                                                                                                                        |
| **5. Geometry–Semantic Alignment: Lack of semantic awareness in geometry reconstruction**                             | Traditional SfM reconstructs geometry only; no semantic labels or semantic consistency across views / time.                                                                                                        | Purely geometric reconstruction cannot capture semantic object / part identity, limits downstream tasks (semantic mapping, object-level editing, scene understanding).            | Introduce **vision–language or semantic embeddings** (e.g. from pretrained semantic models) into SfM / reconstruction pipelines; enforce **semantic consistency constraints** during reconstruction. | **Yes / Partial.** Semantic-aware reconstruction is gaining traction: e.g., the 2024 urban-scene 3D reconstruction method above integrates semantics. ([MDPI][4]) Also in neural rendering, semantic-aware NeRFs are surveyed recently. ([arXiv][5])                                                                                                                                                                                                                                                                                |
| **6. Long-Term Consistency & Memory (temporal consistency across long sequences, memory of past geometry/semantics)** | Most methods optimize per-video, per-scene or per-batch; there is no mechanism to accumulate knowledge across time / sessions — results may drift or be inconsistent in long-term / cross-session reconstructions. | Without memory or temporal modeling, reconstructions from different sessions or times may be inconsistent; dynamic scenes or evolving environments not well handled.              | Integrate **state-space models**, **latent geometry flow**, or **temporal latent modeling** (e.g. recurrent / SSM / memory-augmented models) to enforce consistency across time / sessions.          | **Partial / Emerging.** In the dynamic / neural implicit reconstruction community, some works attempt to handle temporal dynamics and part-level motion: e.g., **MovingParts: Motion-based 3D Part Discovery in Dynamic Radiance Field** discovers part-based motion structure from dynamic scenes. ([arXiv][7]) Also newer works like **VDNeRF: Vision-only Dynamic Neural Radiance Field for Urban Scenes** (2025) aim to handle dynamic urban scenes with joint static/dynamic modeling and camera pose estimation. ([arXiv][8]) |
...
- [2025 - SegSplat: Feed-forward Gaussian Splatting and Open-Set Semantic Segmentation](https://arxiv.org/abs/2511.18386), CVG
