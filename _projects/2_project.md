...
| **Method**                                                                                                                    | **Domain**                                                                                                                                                                                                 | **Feature & Single-Point Solver**                                                               | **Guided Matching** | **Score** |
| ----------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | ------------------- | --------- |
| **AffineGlue**                                                                                                                | 2D→2D image–image                                                                                                                                                                                          | ‣ Affine correspondences (ACs) from SIFT/DoG+AffNet, SuperPoint+AffNet, etc.                    |                     |           |
| ‣ Minimal solver: 1 AC+gravity (1AC+iG) for Essential / Homography; 1 AC+monodepth (1AC+mD) for Essential.                    |                                                                                                                                                                                                            |                                                                                                 |                     |           |
| ‣ New 1 AC+gravity homography solver (1AC+iG-H).                                                                              | After each 1-point solve, re-project each source keypoint into its k–best candidate matches and keep the one with lowest residual (epipolar or homography reprojection).                                   | Number of inliers under a strict residual threshold (optionally weighted by match score ratio). |                     |           |
| **StereoGlue**                                                                                                                | 2D→2D & 2D→3D                                                                                                                                                                                              | ‣ Image features as above (affine-shaped or keypoint+descriptor).                               |                     |           |
| ‣ Minimal solvers: same 1 AC+iG/1AC+mD for E/H, plus P1AC for PnP (2D–3D).                                                    |                                                                                                                                                                                                            |                                                                                                 |                     |           |
| ‣ Also 1 PC+iG for Essential, 1 PC+gravity for Homography.                                                                    | Identical guided matching: for each query feature, pick best candidate among its k–best based on model reprojection (2D→2D or 2D→3D).                                                                      | Same inlier count scoring.                                                                      |                     |           |
| **3D→3D Adaptation**<br/>(e.g. Q-REG)                                                                                         | 3D→3D point cloud–point cloud                                                                                                                                                                              | ‣ Oriented 3D point pairs from learned descriptors (FPFH/SHOT) or GeoTransformer.               |                     |           |
| ‣ Minimal solver: Q-REG builds local quadratic patch at one point→ principal curvatures give relative rotation + translation. | Guided matching via rigid-transform reprojection: keep, for each source point, the target in its descriptor k–best whose Euclidean distance under the estimated transform is minimal and within threshold. | Number of 3D inliers under a strict distance threshold.                                         |                     |           |
