...
Hidden-State Alignment for our model Cell 2.6 -> Or do some `Hidden-State Torch.nn Initialization` by yourself
...
```
🎯 Knowledge Distillation = 3 Competing Objectives:

CE Loss:  ██████████████ (Cross-Entropy with labels)
KL Loss:  ████████░░░░░░ (Knowledge from teacher)  
MSE Loss: ███████░░░░░░░ (Hidden state alignment)

Without LoRA Dropout: One loss dominates → Imbalanced learning
With LoRA Dropout: Forces balanced learning across all objectives

-> Our Work - total_loss = ce_loss + 0.8 * kl_loss + 0.5 * mse_loss

High dropout prevents any single loss from dominating:
- CE loss: Maintains basic speech recognition
- KL loss: Learns from teacher (but not too aggressively) 
- MSE loss: Aligns hidden states (selectively)

Problem without dropout:
output = original_whisper(audio) + lora_adaptation(audio) * 1.0
        ↑ May be overwritten      ↑ Always full modification

Protection with dropout:
output = original_whisper(audio) + lora_adaptation(audio) * dropout_mask
        ↑ Unchanged **dropout** % of time   ↑ Gentle modification **(1-dropout)** % of time
```
...
<p align="left">
  <img src="https://yiruyang2025.github.io/assets/img/project1_3.jpg" alt="Project 1 Visualization" width="45%">
</p>
